...
const EditDiscountPage = ({ params }) => {
  const { createDiscount, getDiscountByCode, updateDiscount } = useDiscount();
  ...
  const {
    ...
    queryKey: ["getDiscount", storeId, code],
    queryFn: () => storeId && code && getDiscountByCode(storeId, code),
    cacheTime: 10 * (60 * 1000),
    staleTime: 5 * (60 * 1000),
    enabled: !!storeId && !!code, // Ensure the query only runs if storeId is available
  });
  ...
  useEffect(() => {
    if (getDiscount) {
      setSelectedStartDateTime(new Date(getDiscount.startTimestamp));
      setSelectedEndDateTime(new Date(getDiscount.endTimestamp));

      setValue("startTimestamp", new Date(getDiscount.startTimestamp));
      setValue("endTimestamp", new Date(getDiscount.endTimestamp));
    }
  }, [getDiscount]);

  console.log("selectedStartDateTime", selectedStartDateTime);
  console.log("selectedEndDateTime", selectedEndDateTime);
  console.log("getDiscount", getDiscount.discountCode);
  ...
  const {
    ...
    defaultValues: {
      discountCode: getDiscount.discountCode,
      value: getDiscount.discountInfo?.value,
      discountType: getDiscount.discountInfo?.type,
      appliesTo: getDiscount.discountInfo?.appliesTo,
      minimumRequirement: getDiscount.minimumRequirement?.type,
      customerEligibility: getDiscount.customerEligibility,
      customerGroups: getDiscount.customerGroups,
      customers: getDiscount.customers,
      limitTotalUsage: getDiscount.usageLimits?.limitTotalUsage,
      totalUsageLimit: getDiscount.usageLimits?.totalUsageLimit,
      limitPerCustomer: getDiscount.usageLimits?.limitPerCustomer,
      perCustomerUsageLimit: getDiscount.usageLimits?.perCustomerUsageLimit,
      categories: getDiscount.categories,
      products: getDiscount.products,
    },
  });
  console.log("amount", getDiscount.minimumRequirement?.amount);
  useEffect(() => {
    if (getDiscount) {
      reset({
        discountCode: getDiscount.discountCode,
        ...
        categories: getDiscount.categories,
        products: getDiscount.products,
        minimumAmount: getDiscount.minimumRequirement?.amount,
        minimumQuantity: getDiscount.minimumRequirement?.quantity,
        startDate: getDiscount.startTime
          ? new Date(getDiscount.startTime)
          : new Date(),
      });
    }
  }, [getDiscount, reset]);
  ...
  const onSubmit = async (data) => {
    ...
    const payload = {
      ...
      startTimestamp: selectedStartDateTime,
      endTimestamp: selectedEndDateTime,
    };
  ...
  if (isDiscountLoading) return <LoadingAnimation />;
